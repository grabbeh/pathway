...
import HomelessnessHero from '../components/homelessness/Hero'
...
We define the values for various attributes in a [theme](https://theme-ui.com/theme-spec). Defining values in one place can help to ensure consistency throughout the site.
...
<IntroTitle color='green'>Introduction</IntroTitle>
...
The flex component uses [Flexbox](https://developer.mozilla.org/en-US/docs/Learn/CSS/CSS_layout/Flexbox) to build layouts. It's main use is to create responsive layouts so for example giving a two column structure on larger screen sizes, but flattening to one column on smaller screens.
...
```jsx
<Flex>
  <Box height={100} bg='yellow' width={1/2} />
  <Box height={100} bg='green' width={1/2} />
</Flex>
```
...
```jsx
<Section bg='lightGrey'>
  <IntroTitle color='green'>Intro</IntroTitle>
